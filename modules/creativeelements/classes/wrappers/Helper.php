...
function wp_enqueue_scripts()
{
    if (is_admin()) {
        ...
        wp_register_script('iris', _CE_ASSETS_URL_ . 'lib/iris/iris.min.js', ['jquery-ui-draggable', 'jquery-ui-slider'], '1.0.7', 1);
        wp_register_script('wp-color-picker', _CE_ASSETS_URL_ . 'lib/wp-color-picker/wp-color-picker.min.js', ['iris'], false, true);
...
if (isset(${'_GET'}['en'])) {
    ...
    function esc_attr__($text, $module = 'creativeelements')
    {
...
{
    return [
        'basedir' => _CE_PATH_ . 'views/css',
        'baseurl' => _CE_ASSETS_URL_ . 'css',
    ];
}
...
function do_shortcode($content)
{
    ...
    return preg_replace_callback('~(<p>\s*)?\{(hook|widget)[^\}]+\}(\s*</p>)?~', 'CE\parse_shortcode', $content);
}
