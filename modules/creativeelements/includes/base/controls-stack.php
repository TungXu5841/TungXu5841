...
abstract class ControlsStack extends BaseObject
{
     ...
     *
     * @since 1.4.0
     * @deprecated 2.3.0 Use `ControlsStack::getItems()` instead
     * @access protected
     * @static
     *
     ...
     */
    protected static function _getItems(array $haystack, $needle = null)
    {
        if ($needle) {
    ...
    public function addControl($id, array $args, $options = [])
    {
        ...
        if (empty($args['type']) || $args['type'] !== ControlsManager::SECTION) {
            ...
            if (null !== $target_section_args) {
                ...
                die(sprintf('%s::%s: Cannot add a control outside of a section (use `start_controls_section`).', get_called_class(), __FUNCTION__));
            }
        }
    ...
    final public function addGroupControl($group_name, array $args = [], array $options = [])
    {
        ...
        if (!$group) {
            die(sprintf('%s::%s: Group "%s" not found.', get_called_class(), __FUNCTION__, $group_name));
        }
    ...
    public function startControlsSection($section_id, array $args = [])
    {
        ...
        if (null !== $this->current_section) {
            die(sprintf('Elementor: You can\'t start a section before the end of the previous section "%s".', $this->current_section['section'])); // XSS ok.
        }
    ...
    public function startControlsTabs($tabs_id, array $args = [])
    {
        if (null !== $this->current_tab) {
            die(sprintf('Elementor: You can\'t start tabs before the end of the previous tabs "%s".', $this->current_tab['tabs_wrapper'])); // XSS ok.
        }
    ...
    public function startControlsTab($tab_id, $args)
    {
        if (!empty($this->current_tab['inner_tab'])) {
            die(sprintf('Elementor: You can\'t start a tab before the end of the previous tab "%s".', $this->current_tab['inner_tab'])); // XSS ok.
        }
    ...
    public function printTemplate()
    {
        ...
        ?>
        <script type="text/html" id="tmpl-elementor-<?= esc_attr($this->getName()) ?>-content">
    ...
    {
        if ($this->injection_point) {
            die('A controls injection is already opened. Please close current injection before starting a new one (use `end_injection`).');
        }
