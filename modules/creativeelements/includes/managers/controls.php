...
class ControlsManager
{
    ...
    private function registerControls()
    {
        ...
        foreach (self::getControlsNames() as $control_id) {
            $control_class_id = str_replace(' ', '', ucwords(str_replace('_', ' ', $control_id)));
            $class_name = __NAMESPACE__ . '\Control' . $control_class_id;
        ...
        foreach (self::getGroupsNames() as $group_name) {
            $group_class_id = str_replace(' ', '', ucwords(str_replace('-', ' ', $group_name)));
            $class_name = __NAMESPACE__ . '\GroupControl' . $group_class_id;
         ...
         * @since 1.0.0
         *
         * @param ControlsManager $this The controls manager.
         */
     ...
     * @param BaseControl $control_instance Control instance, usually the
     *                                       current instance.
     */
    public function registerControl($control_id, BaseControl $control_instance)
    {
     ...
     * @access public
     *
     * @return BaseControl[] Controls list.
     */
     ...
     * @param GroupControlBase $instance Group control instance, usually the
     *                                     current instance.
     *
     * @return GroupControlBase Group control instance.
     */
     ...
     * @access public
     *
     * @param ControlsStack $controls_stack Controls stack.
     */
     ...
     * @access public
     *
     * @param ControlsStack $element      Element stack.
     * @param string         $control_id   Control ID.
    ...
    public function addControlToStack(ControlsStack $element, $control_id, $control_data, $options = [])
    {
        ...
        if ($control_type_instance instanceof BaseDataControl) {
     ...
     *
     * @return bool|WPError True if the stack was removed, False otherwise.
     */
    public function removeControlFromStack($stack_id, $control_id)
    {
        ...
        if (empty($this->stacks[$stack_id]['controls'][$control_id])) {
            return new WPError('Cannot remove not-exists control.');
        }
     ...
     * @param string $control_id Control ID.
     *
     * @return array|WPError The control, or an error.
     */
    public function getControlFromStack($stack_id, $control_id)
    {
        if (empty($this->stacks[$stack_id]['controls'][$control_id])) {
            return new WPError('Cannot get a not-exists control.');
        }
    ...
    public function addCustomCssControls(ControlsStack $controls_stack)
    {
        ...
        $controls_stack->addControl(
            ...
            [
                ...
                'render_type' => 'ui',
                'show_label' => false,
                'description' => __('Use "selector" to target wrapper element. Examples:') .
                    '<pre class="ce-custom-css-hint elementor-panel-alert elementor-panel-alert-info">' . __(
                        "/* For main element */\nselector { color: red; }\n" .
