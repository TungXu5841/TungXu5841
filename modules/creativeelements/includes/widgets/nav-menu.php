    ...
    protected function psMainMenu(array &$nodes, $depth = 0, $ul_class = '')
    {
        ...
        <?php foreach ($nodes as &$node) : ?>
            <li class="<?= sprintf(self::$li_class, $node['type'], $node['page_identifier'], $node['current'] ? ' current-menu-item' : '', $node['children'] ? ' menu-item-has-children' : '') ?>">
                <a class="<?= ($depth ? 'elementor-sub-item' : 'elementor-item') . (stripos($node['url'], '#') !== false ? ' elementor-item-anchor' : '') . ($node['current'] ? ' elementor-item-active' : '') ?>" href="<?= esc_attr($node['url']) ?>"<?= $node['open_in_new_window'] ? ' target="_blank"' : '' ?>>
                    <?= $node['label'] ?>
