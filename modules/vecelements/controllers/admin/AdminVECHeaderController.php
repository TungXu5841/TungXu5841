...
class AdminVECHeaderController extends ModuleAdminController
{
    ...
    public function __construct()
    {
        ...
        $this->_where = "AND a.type = 'header' AND sa.id_shop = " . (int) $this->context->shop->id;
            ...
            'fields' => [
                'CE_HEADER' => [
                    ...
                    'list' => array_merge(
                        ...
                        VECTheme::getOptions('header', $this->context->language->id, $this->context->shop->id)
                    ),
                ],
            ],
            'submit' => [
        ...
        $this->name = 'AdminVECHeader';
    }

    
    public function initToolBarTitle()
    {
        $this->page_header_toolbar_title = $this->l('Header Builder');

        $this->context->smarty->assign('icon', 'icon-list');

        $this->toolbar_title[] = $this->l(
            'add' === $this->display ? 'Add New Template' : ('edit' === $this->display ? 'Edit Template' : 'Header List')
        );
    }
    ...
    public function renderForm()
    {
        ...
        $headerBlock = new VECTheme((int)Tools::getValue('id_vec_theme'));
