 ...
 */
class MobilepanelActivity extends ObjectModel
{
    ...
    public static $definition = [
        'table' => 'vecmobilepanel',
        'primary' => 'id_mobilepanel',
        'multilang' => true,
        'multilang_shop' => true,
        'fields' => [
    ...
    {
        $sql = 'SELECT * FROM `' . _DB_PREFIX_ . 'vecmobilepanel` pr
            LEFT JOIN ' . _DB_PREFIX_ . 'vecmobilepanel_lang prl ON (pr.id_mobilepanel = prl.id_mobilepanel)
    ...
    public static function getAllBlockByShop($id_shop = 1)
    {
        ...
        $sql = 'SELECT * FROM `' . _DB_PREFIX_ . 'vecmobilepanel` pr
            ...
            GROUP BY prl.id_lang, pr.id_mobilepanel
            ORDER BY pr.position';

        $dbResult = Db::getInstance()->executeS($sql);

        foreach ($dbResult as $key => $value) {
            $result[$value['id_lang']][$value['id_mobilepanel']]['title'] = $value['title'];
            $result[$value['id_lang']][$value['id_mobilepanel']]['html_content'] = $value['html_content'];
            $result[$value['id_lang']][$value['id_mobilepanel']]['url'] = $value['link'];
        }
