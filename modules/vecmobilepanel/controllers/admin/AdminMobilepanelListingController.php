 ...
 */
class AdminMobilepanelListingController extends ModuleAdminController
{
    public function __construct()
    {
        parent::__construct();
        if(!(bool)Tools::getValue('ajax'))
         Tools::redirectAdmin($this->context->link->getAdminLink('AdminModules').'&configure=vecmobilepanel');
    ...
    public function displayAjaxChangeBlockStatus()
    {
        ...
        $whereCondition = 'id_mobilepanel = ' . $psreassuranceId;

        $updateResult = Db::getInstance()->update('vecmobilepanel', $dataToUpdate, $whereCondition);
    ...
    public function displayAjaxDeleteBlock()
    {
        ...
        $blockPSR = Db::getInstance()->getRow('SELECT * FROM ' . _DB_PREFIX_ . 'vecmobilepanel WHERE `id_mobilepanel` = ' . (int) $idPSR);
        if (!empty($blockPSR)) {
            $result = true;
            // Remove Custom icon
            ...
            if ($result) {
                $result = Db::getInstance()->delete('vecmobilepanel_lang', 'id_mobilepanel = ' . (int) $idPSR);
            }
            // Remove Block
            if ($result) {
                $result = Db::getInstance()->delete('vecmobilepanel', 'id_mobilepanel = ' . (int) $idPSR);
            }
        }
    ...
    public function displayAjaxSaveBlockContent()
    {
        ...
        $blockPsr = new MobilepanelActivity($id_block);
        if (!$id_block) {
            // Last position
            $blockPsr->position = Db::getInstance()->getValue('SELECT MAX(position) AS max FROM ' . _DB_PREFIX_ . 'vecmobilepanel');
        ...
        $blockPsr->icon = $picto;
        if (Tools::getValue('selected_icon')) {
    ...
    public function displayAjaxUpdatePosition()
    {
        ...
        if (!empty($blocks) && is_array($blocks)) {
            foreach ($blocks as $key => $id_block) {
                ...
                $whereCondition = 'id_mobilepanel = ' . (int) $id_block;
                $updateResult = (bool) Db::getInstance()->update('vecmobilepanel', $dataToUpdate, $whereCondition);
