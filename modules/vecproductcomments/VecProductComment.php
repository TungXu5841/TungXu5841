...
class VecProductComment extends ObjectModel
{
	public $id;
	...
	public static function getByProduct($id_product, $p = 1, $n = null, $id_customer = null)
	{
		if (!Validate::isUnsignedId($id_product))
			return false;
		$validate = Configuration::get('VEC_PRODUCT_COMMENTS_MODERATE');
		$p = (int)$p;
		$n = (int)$n;
		if ($p <= 1)
			$p = 1;
		...
		$cache_id = 'VecProductComment::getByProduct_'.(int)$id_product.'-'.(int)$p.'-'.(int)$n.'-'.(int)$id_customer.'-'.(bool)$validate;
	...
	{
		$cache_id = 'VecProductComment::getByCustomer_'.(int)$id_product.'-'.(int)$id_customer.'-'.(bool)$get_last.'-'.(int)$id_guest;
		if (!Cache::isStored($cache_id))
		{
	...
	public static function getGradeByProduct($id_product, $id_lang)
	{
		...
		FROM `'._DB_PREFIX_.'vec_product_comment` pc
		LEFT JOIN `'._DB_PREFIX_.'vec_product_comment_grade` pcg ON (pcg.`id_product_comment` = pc.`id_product_comment`)
		LEFT JOIN `'._DB_PREFIX_.'vec_product_comment_criterion` pcc ON (pcc.`id_product_comment_criterion` = pcg.`id_product_comment_criterion`)
		LEFT JOIN `'._DB_PREFIX_.'vec_product_comment_criterion_lang` pccl ON (pccl.`id_product_comment_criterion` = pcg.`id_product_comment_criterion`)
		WHERE pc.`id_product` = '.(int)$id_product.'
	...
	{
		/* Get all grades */
		$grades = VecProductComment::getGradeByProduct((int)$id_product, (int)$id_lang);
		$total = VecProductComment::getGradedCommentNumber((int)$id_product);
	...
	public static function getCommentNumber($id_product)
	{
		if (!Validate::isUnsignedId($id_product))
			return false;
		$validate = (int)Configuration::get('VEC_PRODUCT_COMMENTS_MODERATE');
		$cache_id = 'VecProductComment::getCommentNumber_'.(int)$id_product.'-'.$validate;
		if (!Cache::isStored($cache_id))
		{
	...
	public function validate($validate = '1')
	{
		...
		UPDATE `'._DB_PREFIX_.'vec_product_comment` SET
