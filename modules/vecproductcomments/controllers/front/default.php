...
// Include Module
include_once(dirname(__FILE__).'/../../vecproductcomments.php');
// Include Models
include_once(dirname(__FILE__).'/../../VecProductComment.php');
include_once(dirname(__FILE__).'/../../VecProductCommentCriterion.php');

class VecProductCommentsDefaultModuleFrontController extends ModuleFrontController
{
	...
	{
		$module_instance = new VecProductComments();
		...
		if (!$this->context->customer->id && !Configuration::get('VEC_PRODUCT_COMMENTS_ALLOW_GUESTS'))
		...
		if (!count($errors))
		{
			$customer_comment = VecProductComment::getByCustomer(Tools::getValue('id_product'), $id_customer, true, $id_guest);
			if (!$customer_comment || ($customer_comment && (strtotime($customer_comment['date_add']) + (int)Configuration::get('VEC_PRODUCT_COMMENTS_MINIMAL_TIME')) < time()))
			{

				$comment = new VecProductComment();
				...
				{
					$grade_sum += $grade;
					$product_comment_criterion = new VecProductCommentCriterion($id_product_comment_criterion);
				...
				$result = true;
				Tools::clearCache(Context::getContext()->smarty, $this->getTemplatePath('vecproductcomments-reviews.tpl'));
			}
			else
			{
				$result = false;
				$errors[] = $module_instance->l('Please wait before posting another comment').' '.Configuration::get('VEC_PRODUCT_COMMENTS_MINIMAL_TIME').' '.$module_instance->l('seconds before posting a new comment');
			}
		}
		else
			$result = false;
