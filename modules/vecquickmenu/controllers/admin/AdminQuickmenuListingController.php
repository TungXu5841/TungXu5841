...
class AdminQuickmenuListingController extends ModuleAdminController
{
    public function __construct()
    {
        ...
        if(!(bool)Tools::getValue('ajax'))
         Tools::redirectAdmin($this->context->link->getAdminLink('AdminModules').'&configure=vecquickmenu');
    ...
    public function displayAjaxChangeBlockStatus()
    {
        ...
        $updateResult = Db::getInstance()->update('vecquickmenu', $dataToUpdate, $whereCondition);
    ...
    public function displayAjaxDeleteBlock()
    {
        ...
        $blockPSR = Db::getInstance()->getRow('SELECT * FROM ' . _DB_PREFIX_ . 'vecquickmenu WHERE `id_quickmenu` = ' . (int) $idPSR);
        if (!empty($blockPSR)) {
            $result = true;
            // Remove Custom icon
            ...
            if ($result) {
                $result = Db::getInstance()->delete('vecquickmenu_lang', 'id_quickmenu = ' . (int) $idPSR);
            }
            // Remove Block
            if ($result) {
                $result = Db::getInstance()->delete('vecquickmenu', 'id_quickmenu = ' . (int) $idPSR);
            }
        }
    ...
    public function displayAjaxSaveBlockContent()
    {
        ...
        if (!$id_block) {
            // Last position
            $blockPsr->position = Db::getInstance()->getValue('SELECT MAX(position) AS max FROM ' . _DB_PREFIX_ . 'vecquickmenu');
    ...
    public function displayAjaxUpdatePosition()
    {
        ...
        if (!empty($blocks) && is_array($blocks)) {
            foreach ($blocks as $key => $id_block) {
                ...
                $updateResult = (bool) Db::getInstance()->update('vecquickmenu', $dataToUpdate, $whereCondition);
