...
include_once(_PS_MODULE_DIR_.'vecquickmenu/classes/QuickmenuActivity.php');

class vecQuickMenu extends Module implements WidgetInterface
{
    ...
    public function __construct()
    {
        ...
        $this->displayName = $this->l('Vec - Quick menu mobile');
        ...
        $this->templateFile = 'module:vecquickmenu/views/templates/hook/vecquickmenu.tpl';
    }
    ...
    public function install()
    {
        // SQL
        $sqlQueries = [];
        $sqlQueries[] = ' CREATE TABLE IF NOT EXISTS `' . _DB_PREFIX_ . 'vecquickmenu` (
        ...
        $sqlQueries[] = 'CREATE TABLE IF NOT EXISTS `' . _DB_PREFIX_ . 'vecquickmenu_lang` (
        ...
        $sqlQueries[] = 'INSERT INTO ' . _DB_PREFIX_ . 'vecquickmenu (icon, custom_icon, status, position, id_shop, type_content, id_cms) VALUES '
        ...
        foreach (Language::getLanguages(false) as $lang) {
            $sqlQueries[] = 'INSERT INTO ' . _DB_PREFIX_ . 'vecquickmenu_lang (id_quickmenu, id_lang, id_shop, title, html_content, link) VALUES '
    ...
    {
        // SQL
        $sql = 'DROP TABLE IF EXISTS `' . _DB_PREFIX_ . 'vecquickmenu`, `' . _DB_PREFIX_ . 'vecquickmenu_lang`';
    ...
    public function hookHeader(){
        ...
        return $this->display(__FILE__, 'views/templates/hook/vecquickmenu-header.tpl');
    }
    /**
    ...
    {
        if (!$this->isCached($this->templateFile, $this->getCacheId('vecquickmenu'))) {
            $this->smarty->assign($this->getWidgetVariables($hookName, $configuration));
        }

        return $this->fetch($this->templateFile, $this->getCacheId('vecquickmenu'));
    }
