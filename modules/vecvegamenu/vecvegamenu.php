...
include_once(_PS_MODULE_DIR_.'vecvegamenu/src/vecVegamenuClass.php');
include_once(_PS_MODULE_DIR_.'vecvegamenu/src/vecVegamenuSubmenuClass.php');
include_once(_PS_MODULE_DIR_.'vecvegamenu/src/vecVegamenuSubmenuRowClass.php');
include_once(_PS_MODULE_DIR_.'vecvegamenu/src/vecVegamenuSubmenuColumnClass.php');
include_once(_PS_MODULE_DIR_.'vecvegamenu/src/vecVegamenuSubmenuItemClass.php');
include_once(_PS_MODULE_DIR_.'vecvegamenu/sql/vecSampleDataVMenu.php');

class VecVegamenu extends Module 
{	
	...
	{	
		...
		$this->bootstrap = true;
		parent::__construct();
		$this->displayName = $this->l('Vec - Vertical Menu');
		$this->description = $this->l('Vertical megamenu custommer module');
	...
	public function install()
	{
		if (parent::install() &&
			...
			$this->registerHook('actionObjectLanguageAddAfter'))
			{
				...
				$sample_data = new VecSampleDataVMenu();
	...
	public function installTab()
    {
        $tab = new Tab();
        $tab->active = 1;
        $tab->class_name = "AdminVecVegamenu";
        $tab->name = array();
	...
	{
		$tab = new Tab((int)Tab::getIdFromClassName('AdminVecVegamenu'));
        $tab->delete();	
	...
	public function getContent()
	{
		...
		elseif (Tools::isSubmit('buildMenu') && Tools::isSubmit('id_vecvegamenu_item') )
		{
			$this->_postProcess();
	...
	{	
		...
		{
			$info_menus[$key]['status'] = $this->displayStatus($info_menu['id_vecvegamenu_item'], $info_menu['active']);
		}
	...
	public function displayStatus($id_vecvegamenu_item, $active)
	{
		...
		$html = '<a class="btn '.$class.'" href="'.AdminController::$currentIndex.
			'&configure='.$this->name.'
				...
				&changeStatus&id_vecvegamenu_item='.(int)$id_vecvegamenu_item.'" title="'.$title.'"><i class="'.$icon.'"></i> '.$title.'</a>';
	...
	public function getMenuInfo($active = null)
	{
		...
		return Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			LEFT JOIN '._DB_PREFIX_.'vecvegamenu_item_lang pil ON pi.id_vecvegamenu_item = pil.id_vecvegamenu_item 
			LEFT JOIN '._DB_PREFIX_.'vecvegamenu_item_shop pis ON pi.id_vecvegamenu_item = pis.id_vecvegamenu_item 
			WHERE pis.id_shop = '.$id_shop.' AND pil.id_lang = '.$id_lang.($active ? ' AND pi.`active` = 1' : ' ').' ORDER BY pi.position ASC, pi.id_vecvegamenu_item ASC'
		);
	}
	protected function _postProcess()
	{
		...
		{
			$this->_clearCache('vecvegamenu.tpl');

			if (Tools::getValue('id_vecvegamenu_item'))
			{
				$menu_item = new VecVegamenuClass((int)Tools::getValue('id_vecvegamenu_item'));
				if (!Validate::isLoadedObject($menu_item))
				{
					$this->html .= $this->displayError($this->l('Invalid id_vecvegamenu_item'));
					return false;
				}
			}
			else{
				$menu_item = new VecVegamenuClass();
			}
			...
			if (!$errors)
			{
				if (!Tools::getValue('id_vecvegamenu_item'))
				{
					$menu_item->position = VecVegamenuClass::getLastPosition()+1;
					if (!$menu_item->add())
					{
		...
		elseif (Tools::isSubmit('changeStatus') && Tools::isSubmit('id_vecvegamenu_item'))
		{
			$this->_clearCache('vecvegamenu.tpl');
			$menu = new VecVegamenuClass((int)Tools::getValue('id_vecvegamenu_item'));
			if ($menu->active == 0)
				$menu->active = 1;
			else
				$menu->active = 0;
		...
		{
			$this->_clearCache('vecvegamenu.tpl');
			$menu_item = new VecVegamenuClass((int)Tools::getValue('delete_id_menu'));
			$res = $menu_item->delete();
			if (!$res)
				$this->html .= $this->displayError('Could not delete.');
			else{
				Tools::redirectAdmin($this->context->link->getAdminLink('AdminModules', true).'&conf=1&configure='.$this->name.'&tab_module='.$this->tab.'&module_name='.$this->name.'&id_vecvegamenu_item='.Tools::getValue('id_vecvegamenu_item'));
				$this->generateCss();
			}
		}
		elseif(Tools::isSubmit('submitSubmenu')){
			$submenu_item = new VecVegamenuSubmenuClass((int)Tools::getValue('id_vecvegamenu_item'));	
			
			$submenu_item->id_vecvegamenu_item = (int)Tools::getValue('id_vecvegamenu_item');
	...
	public function renderAddForm()
	{
		...
		if (isset($id_vecvegamenu_item))
			$menu = new VecVegamenuClass($id_vecvegamenu_item);
		else
			$menu = new VecVegamenuClass();
	...
	public function renderBuildMenu()
	{
		...
		$languages = $this->context->controller->getLanguages();
		$id_vecvegamenu_item = Tools::getValue('id_vecvegamenu_item', 1);
		$submenu_info = $this->getSubmenuInfo($id_vecvegamenu_item);
		$info_rows = $this->getRowInfo($id_vecvegamenu_item);
		...
		$this->context->smarty->assign(
			array(
				'id_vecvegamenu_item' => $id_vecvegamenu_item,
				'token' => Tools::getAdminTokenLite('AdminModules'),
				'info_rows' => $info_rows,
	...
	public function getSubmenuInfo($id_menu, $active = null)
	{
		...
		$row_infos_rs = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow('
			...
			WHERE ps.id_vecvegamenu_item = '.$id_menu.($active ? ' AND ps.`active` = 1' : ' ')
		);
	...
	public function getRowInfo($id_menu, $active = null)
	{
		...
		$row_infos_rs = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT sr.*
			FROM '._DB_PREFIX_.'vecvegamenu_submenu_row sr
			WHERE sr.id_vecvegamenu_item = '.$id_menu.($active ? ' AND sr.`active` = 1' : '').' ORDER BY sr.position ASC'
		);
		if (is_array($row_infos_rs) && count($row_infos_rs) > 0)
			$row_infos = $row_infos_rs;
		return $row_infos;
	...
	public function getColInfo($id_row, $active = null, $is_backend = true)
	{
		...
		$cols_result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			FROM '._DB_PREFIX_.'vecvegamenu_submenu_column sc
			LEFT JOIN '._DB_PREFIX_.'vecvegamenu_submenu_column_lang scl ON (sc.id_vecvegamenu_submenu_column = scl.id_vecvegamenu_submenu_column)
			WHERE  sc.id_row = '.$id_row.'  AND scl.`id_lang` = '.$id_lang.'  AND sc.`active` = 1
			ORDER BY sc.position ASC, sc.id_vecvegamenu_submenu_column ASC'
		);
		...
		if (is_array($col_infos) && count($col_infos) > 0)
			foreach ($col_infos as $key => $col_info)
			{
				...
				$sub_menu_infos = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
					...
					LEFT JOIN '._DB_PREFIX_.'vecvegamenu_submenu_item_lang psil ON psi.id_vecvegamenu_submenu_item = psil.id_vecvegamenu_submenu_item
					WHERE psi.id_vecvegamenu_submenu_column = '.$col_info['id_vecvegamenu_submenu_column'].' AND psil.id_lang = '.$id_lang.($active ? ' AND psi.`active` = 1' : '').' ORDER BY psi.position ASC, psi.id_vecvegamenu_submenu_item ASC');
				if (is_array($sub_menu_infos) && count($sub_menu_infos) > 0)
				{	
					foreach ($sub_menu_infos as $key1 => $sub_menu_info)
					{	
						$id_vecvegamenu_item = Tools::getValue('id_vecvegamenu_item', 1);
						//if (isset($id_vecvegamenu_item) && $id_vecvegamenu_item > 0 && $is_backend)
	...
	public function getSubMenu($id_vecvegamenu_item)
	{
		$submenu_info = array();
		$info_rows = array();
		$submenu_config = $this->getSubmenuInfo($id_vecvegamenu_item, true);
		if (is_array($this->getRowInfo($id_vecvegamenu_item, true)) && count($this->getRowInfo($id_vecvegamenu_item, true)) > 0)
		{
			$info_rows = $this->getRowInfo($id_vecvegamenu_item, true);			
			foreach ($info_rows as $key => $info_row){
	...
	{
		$req = 'SELECT pi.`id_vecvegamenu_item` as id_vecvegamenu_item
				FROM `'._DB_PREFIX_.'vecvegamenu_item` pi
				WHERE pi.`id_vecvegamenu_item` = '.(int)$id;
	...
	public function hookDisplayVegamenu(array $params)
	{
		$id_lang = (int)$this->context->language->id;
		$id_shop = (int)Context::getContext()->shop->id;
		...
		$group_cat_result = array();
			...
			foreach ($menus as $menu)
			{
				...
				if($menu['submenu_type'] == 0 || $menu['submenu_type'] == 1){
					$sub_menu = $this->getSubMenu($menu['id_vecvegamenu_item']);
			...
			$this->context->smarty->assign(array(
				'menus' => $new_menus,
				'title' => isset($params['title']) ? $params['title'] : $this->l('Categories'),
			));
		
		return $this->display(__FILE__, 'vecvegamenu.tpl', $this->getCacheId('vecvegamenu'));
	}
	...
	public function generateCss()
	{
		...
		if (Shop::getContext() == Shop::CONTEXT_SHOP)
			$my_file = $this->local_path.'views/css/vecvegamenu_s_'.(int)$this->context->shop->getContextShopID().'.css';
